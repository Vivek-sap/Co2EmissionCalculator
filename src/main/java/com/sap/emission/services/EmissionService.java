...
public class EmissionService {
	    ...
	    public EmissionService(@Value("${openrouteservice.url}") String openRouteUrl,  TransportationService transportationService) {
	    ...
	    public EmissionResult calculateDistance(String start, String end, String transportMethod) throws Exception {
	    	if (transportationService.isValidTransportMethod(transportMethod)) {
	    		int[] source = transportationService.parseCoordinateData(fetchCoordinateByCity(start));
	    		int[] destination = transportationService.parseCoordinateData(fetchCoordinateByCity(start));
	    	}
	    ...
	    private String fetchCoordinateByCity(String text) {
	    	 ...
	    	 String layers = "locality";
	    	  ...
	    	  try {
	 			 res =  restTemplate.getForObject("/gecode/search/api_key/{api_key}/text/{text}/layers/{layers}", String.class, api_key,
	 					  text,layers);
	 			validateResult(res);
